...
ArrayGeralAggregate = np.column_stack((nameAggOlindaArray, sizeAggOlindaArray, usedAggOlindaArray, dispoAggOlindaArray, percAggOlindaArray))
...
somatoriaStorage= somaLiqOlinda 
...
for v in myresultVolumesOlinda:
   ...
   # aggVol = v[12].upper(#)
    aggVol = v[12]
    arrayAggVolOlinda = np.append(arrayAggVolOlinda, aggVol)
...
# arrayVolumesOlinda = np.column_stack((nameArrayOlinda, sizeArrayOlinda, usedArrayOlinda, avaArrayOlinda, arrayAggVolOlinda))
...
for d in myresultDivisoes:
    ...
    # Creating the HTML file
    # varDiv = 'templates/teste/' + divOlinda + '.html'
    # if os.path.exists(varDiv):
    #     os.remove(varDiv)
    ...
    html = divOlinda
    html = generate_html_D(table, divOlinda, mergeTotalDivOlinda, mergeUsadoDivOlinda, mergeDisponivelDivOlinda)
